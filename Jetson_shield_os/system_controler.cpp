...
constexpr TickType_t kControllerQueueWaitTicks = pdMS_TO_TICKS(jetson_cfg::kControllerTickMs);
...
void SystemController::syncLcd2Environment() {
    ...
    const int16_t ledBrightnessPercent = map(_led.getBrightness(), 0, 255, 0, 100);
...
void SystemController::controllerTaskLoop() {
    for (;;) {
        ...
        const bool hasMessage = (xQueueReceive(_messageQueue, &message, kControllerQueueWaitTicks) == pdTRUE);
        if (hasMessage) {
